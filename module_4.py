...
def extract_features(features, feature_names):
    ...
    chosen_idx = chosen_ac_pi_a  # module 5
    features = features[:, chosen_idx]
    feature_names = feature_names[chosen_idx]
    return (features, feature_names)

# classifier
def module_4_model_a_creation(model_type, model_a_mimic_cohort, model_a_eicu_cohort):
    ...
    classifier_parameter = classifier_parameter_a # module 5
    resampling_dict = resampling_dict_a     # module 5

    data_mimic_a = pd.read_csv(fr'{model_a_mimic_cohort}')
    data_eicu_a = pd.read_csv(fr'{model_a_eicu_cohort}')
    data_frame = pd.concat([data_mimic_a,data_eicu_a], ignore_index=True)
    X, y = get_X_and_y(data_frame, "target", "identifier")
    ...
    # classifier = RandomForestClassifier(**classifier_parameter) # module 5
    classifier.fit(X, y)
    
    with open("chosen_feature_names_a","wb") as fea:
        pickle.dump(feature_names, fea)
    
    with open("imputer_fit_a","wb") as imp_p:
        pickle.dump(imputer_fit, imp_p)
    with open("scaler_a","wb") as scaler_a:
        pickle.dump(scaler, scaler_a)
    with open("classifier_a","wb") as clf_a:
        pickle.dump(classifier, clf_a)
...
module_4_model_a_creation("a", "./processed_data_mimic_task_a.csv", "./processed_data_eicu.csv")